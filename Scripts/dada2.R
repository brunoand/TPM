...
fnFs <- sort(list.files(path, pattern="_R1_001.fq", full.names = TRUE))
fnRs <- sort(list.files(path, pattern="_R2_001.fq", full.names = TRUE))
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, maxN=0, maxEE=c(3,5), rm.phix=TRUE, compress=TRUE, multithread=TRUE)
...
mergers <- mergePairs(dadaFs, derepFs, dadaRs, derepRs, verbose=TRUE, minOverlap = 15)
